...
# Config
# ----------------------------
DATASET_JSON_PATH = "../data_filteration/gqa/passed_questions_2.json"        # your big JSON (dict keyed by question_id)
PROMPT_TXT_PATH = "./prompt/updated_gqa_prompt.txt"            # your prompt template file
...
SYSTEM_PROMPT = """
        ...
        You must strictly follow all instructions in the user prompt.

        Rules:
        - Base all decisions ONLY on the provided image and scene graph.
        - Do NOT hallucinate objects that are not explicitly present.
        - If an object is not in the scene graph, it must be treated as not present in the image.
        - Carefully verify object existence before selecting distractors.
        - When selecting a segment, ensure it is realistically writable.
        - If any required condition is not satisfied, return False exactly as instructed.
...
def fill_prompt(template: str, *, question: str, correct_answer: str, objects_list: List[str]) -> str:
    """
    ...
    return (
        ...
        .replace("{{OBJECTS_LIST}}", json.dumps(objects_list, ensure_ascii=False))
    )

def build_user_content(prompt_text: str, orig_img: Image.Image, seg_img: Image.Image) -> List[Dict[str, Any]]:
    """
    Produces a multimodal user content list: text + 2 images
    """
    return [
        {"type": "text", "text": prompt_text},
        {"type": "image_url", "image_url": {"url": pil_to_data_url(orig_img, mime="image/jpeg", fmt="JPEG")}},
        {"type": "image_url", "image_url": {"url": pil_to_data_url(seg_img, mime="image/jpeg", fmt="JPEG")}},
    ]
...
def main():
    ...
    seg_dir = Path(SEG_IMG_DIR)

    prompt_template = load_prompt_template(PROMPT_TXT_PATH)

    with open(DATASET_JSON_PATH, "r", encoding="utf-8") as f:
        data = json.load(f)  # dict: question_id -> record
    ...
    for i, qid in enumerate(keys):
        ...
        objects_list = set([rec['scene_graph']['objects'][obj_id]['name'] for obj_id in rec['scene_graph']['objects']])
        objects_list = list(objects_list)  # convert to list for JSON serialization
        image_id = str(rec.get("imageId", "")).strip()

        if not image_id:
            print(f"[WARN] Missing imageId for qid={qid}, skipping")
            skipped += 1
            continue

        orig_path = resolve_image_path(orig_dir, image_id, ORIG_EXTS)
        seg_path = resolve_image_path(seg_dir, image_id, SEG_EXTS)

        if not orig_path or not seg_path:
            print(f"[WARN] Missing files for imageId={image_id} (qid={qid}) "
                  f"orig={orig_path} seg={seg_path}, skipping")
            skipped += 1
            continue

        orig_img = safe_open_image(orig_path)
        seg_img = safe_open_image(seg_path)
        ...
        temp["body"]["messages"][1]["content"] = build_user_content(prompt_text, orig_img, seg_img)
