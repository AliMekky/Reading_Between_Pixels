...
Mismatch handling (MATCHES your IG script behavior):
- DOES NOT SKIP on mismatch.
- Always builds mapping with include_newline_tokens=True (same as IG script).
- If token counts mismatch:
    - prints WARNING
    - appends qid to out_dir/mismatched_qids.txt
    - still SAVES the NPZ (with mapping + placeholders), but marks meta["mapping_mismatch"]=True
"""
...
# -----------------------------
# Packed image-token mapping logic (same as IG script)
# -----------------------------
def get_anyres_image_grid_shape(image_size_hw: Tuple[int, int], grid_pinpoints, tile_size: int) -> Tuple[int, int]:
    best_h, best_w = select_best_resolution(list(image_size_hw), grid_pinpoints)
    return best_h // tile_size, best_w // tile_size
...
def main():
    ...
    # Outputs are stored in out_dir/variant/qid/
    ...
    for i in tqdm(range(len(ds)), desc=f"Caching gen-token attn (chat template) [{args.variant}]"):
        ...
        # evaluator-style chat template (FIX: tokenize spelled correctly)
        conversation = [
        ...
        # MATCH IG EXACTLY: include_newline_tokens=True (no toggling)
        packed_mapping = build_packed_image_token_map_for_one_image(
            model=model,
            original_size_hw=(orig_h, orig_w),
            num_views_provided=num_views,
        ...
        meta = {
            ...
            "option_meta": option_meta,
            "original_image_size_hw": [orig_h, orig_w],
            "num_views_provided": int(num_views),
            "include_newline_tokens": True,  # fixed, IG-style
            **gen_info,
        }
        ...
        # SAVE ALWAYS (even if mismatch), IG-style
        save_npz_sample(
            args.out_dir,
            qid,
            args.variant,
