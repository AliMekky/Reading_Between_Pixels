...
#         plt.plot(x, m, label=name, color=color)
...
# Nunito with DejaVu Sans fallback for arrows/special chars
...
def _apply_acl_style(ax):
    ...
    ax.yaxis.set_label_coords(-0.1, 0.5)   # push ylabel left so it never clips
...
def plot_notext_only_with_ci(
    ...
    # REPLACE in plot_notext_only_with_ci:
    colors = {
        "text region":             '#0072B2',
        "correct object region":   '#009E73',
        "misleading object region":'#D55E00',
...
def main():
    ...
    for stream in streams:
        ...
        plot_notext_only_with_ci(
            ...
            series=series_notext,
            out_path=out_root / f"subset_notext_only_{stream}_{args.subset}.pdf",
            ylabel="Avg attn / token (region)\n/ avg attn / token (image)",
            ci=args.ci,
        )
