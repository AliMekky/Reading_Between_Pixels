...
Outputs:
  ...
  When --subset combined_mg is used (variant=misleading_groundable):
    combined_mg_<stream>.pdf   – 2-row x 1-col figure
      (a) Harmful text caused the flip   [fooled subset]
      (b) Harmful text, stayed correct   [robust subset]
"""
...
# ── ACL-ready plot defaults ──────────────────────────────────────────────────
ACL_COL_W   = 3.5          # inches – one column of a 2-col ACL paper
ACL_FIG_H   = 2.6          # inches – comfortable height per subplot row
FONT_AXIS   = 7            # axis label font size (pt)
FONT_TICK   = 6.5          # tick label font size (pt)
FONT_LEGEND = 6.5          # legend font size (pt)
FONT_PANEL  = 7            # panel label "(a)" / "(b)" font size (pt)
LINE_W      = 1.4          # main line width
ALPHA_CI    = 0.18         # CI shading transparency
DPI_OUT     = 300          # publication-quality DPI
...
def _apply_acl_style(ax):
    """Apply consistent ACL-paper styling to an Axes object."""
    ax.tick_params(axis="both", labelsize=FONT_TICK)
    ax.xaxis.label.set_size(FONT_AXIS)
    ax.yaxis.label.set_size(FONT_AXIS)
    ax.yaxis.set_label_coords(-0.18, 0.5)   # push ylabel left so it never clips
    ax.spines[["top", "right"]].set_visible(False)


def _draw_series(ax, x, series, ci):
    """Draw all region lines + CI bands onto an existing Axes."""
    colors = {
        "text region":              "#0072B2",
        "correct object region":    "#009E73",
        "misleading object region": "#D55E00",
    }
    for name, mat in series.items():
        m, lo, hi = nanmean_ci(mat, ci=ci)
        color = colors.get(name, None)
        ax.plot(x, m, label=name, color=color, linewidth=LINE_W)
        ax.fill_between(x, lo, hi, color=color, alpha=ALPHA_CI)
...
def plot_three_lines_with_ci(
    ...
    fig, ax = plt.subplots(figsize=(ACL_COL_W, ACL_FIG_H))
    ...
    ax.set_xlabel("Layer", fontsize=FONT_AXIS)
    ax.set_ylabel(ylabel, fontsize=FONT_AXIS)
    # plt.title(title)
    ax.legend(fontsize=FONT_LEGEND, frameon=False, loc="best", handlelength=1.5)
    _apply_acl_style(ax)

    fig.tight_layout(pad=0.4)
    fig.savefig(out_path, dpi=DPI_OUT, bbox_inches="tight")
    plt.close(fig)
...
def plot_combined_mg_subsets(
    x: np.ndarray,
    series_fooled: Dict[str, np.ndarray],   # region -> (N, L)  fooled subset
    series_robust: Dict[str, np.ndarray],   # region -> (N, L)  robust subset
    out_path: Path,
    ylabel: str,
    ci: float = 95.0,
    # ── highlight band ────────────────────────────────────────────────────────
    highlight_spans: Optional[List[Tuple[float, float]]] = None,
    # e.g. [(10, 15)] draws one band; [(10,15),(20,25)] draws two
    highlight_color: str = "#D55E00",   # band fill colour (default: orange)
    highlight_alpha: float = 0.10,      # band transparency
    highlight_label: Optional[str] = "region of interest",  # None → no legend entry
):
    """
    2-row x 1-col figure for misleading_groundable, shared x-axis.
      (a) top row    – fooled subset  – "Harmful text caused the flip"
      (b) bottom row – robust subset  – "Harmful text, stayed correct"
    A single shared legend is placed below both panels.
    Optional highlight_spans draws shaded vertical bands on every panel.
    Only the bottom panel carries the x-axis label ("Layer").
    """
    fig, axes = plt.subplots(
        nrows=2, ncols=1,
        figsize=(ACL_COL_W, ACL_FIG_H * 2),   # stack two panels
        sharex=True,
    )

    panel_labels = [
        "(a) Harmful text caused the flip",
        "(b) Harmful text, stayed correct",
    ]
    all_series = [series_fooled, series_robust]

    for i, (ax, series, panel_lbl) in enumerate(zip(axes, all_series, panel_labels)):
        # ── shaded highlight bands (drawn first so lines sit on top) ──────────
        if highlight_spans:
            for j, (x0, x1) in enumerate(highlight_spans):
                # only attach the legend label to the very first band on the
                # first panel so it appears exactly once in the shared legend
                lbl = highlight_label if (i == 0 and j == 0) else None
                ax.axvspan(x0, x1,
        ...
        _draw_series(ax, x, series, ci)

        # panel label above the subplot with controllable vertical padding
        ax.set_title(
            panel_lbl,
            fontsize=FONT_PANEL,
            fontweight="bold",
            loc="left",
            pad=6,          # points of space between the axes top and the label
        )

        ax.set_ylabel(ylabel, fontsize=FONT_AXIS)
        _apply_acl_style(ax)

    # shared x-axis label only on the bottom panel (top ticks suppressed by sharex)
    axes[-1].set_xlabel("Layer", fontsize=FONT_AXIS)

    # ── single shared legend below both panels ────────────────────────────────
    # collect handles/labels from the top panel (all line artists live there)
    ...
    fig.legend(
        ...
        frameon=False,
        loc="lower center",
        ncol=2,                 # two columns keeps it compact under a narrow figure
        handlelength=1.5,
        bbox_to_anchor=(0.5, -0.02),   # just below the figure bottom
    )

    fig.tight_layout(pad=0.4, h_pad=0.6)
    # extra bottom margin so the legend isn't clipped
...
def main():
    ...
    parser.add_argument(
        ...
        type=str,
        default="combined_mg",
        choices=["helped", "fooled", "all", "robust", "wrongly_consistent", "combined_mg"],
        help=(
            "Which subset to aggregate. Non-selected qids are kept as all-NaN (no skip). "
            "Use 'combined_mg' to produce a 2-row x 1-col subfigure (fooled + robust) for "
            "misleading_groundable in one pass."
        ),
    )
    ...
    # ── combined_mg mode: maintain two parallel stores (fooled + robust) ──────
    is_combined = args.subset == "combined_mg"
    ...
    # secondary stores used only when --subset combined_mg
    store_V2: Dict[str, Dict[str, List[np.ndarray]]] = {s: {r: [] for r in region_names} for s in streams}
    store_N2: Dict[str, Dict[str, List[np.ndarray]]] = {s: {r: [] for r in region_names} for s in streams}
    ...
    counts: Dict[str, int] = {
        ...
        "N_selected": 0,
        "N_selected_fooled": 0,   # populated when combined_mg
        "N_selected_robust": 0,   # populated when combined_mg
    ...
    for qid in qids:
        ...
        if pv is None or pn is None:
            for s in streams:
                for r in region_names:
                    ...
                    if is_combined:
                        store_V2[s][r].append(nan_vec())
                        store_N2[s][r].append(nan_vec())
        ...
        # ── subset assignment ─────────────────────────────────────────────────
        if is_combined:
            # store1 = fooled, store2 = robust; each qid routes to its own store
            sel_fooled = is_fooled(dv["meta"], dn["meta"], args.variant)
            sel_robust = is_robust(dv["meta"], dn["meta"], args.variant)
            is_selected = sel_fooled or sel_robust   # used only for selected_mask
            if sel_fooled:
                counts["N_selected_fooled"] += 1
            if sel_robust:
                counts["N_selected_robust"] += 1
            counts["N_selected"] += int(is_selected)
        ...
        if not is_combined and is_selected:
            counts["N_selected"] += 1
        ...
        if is_combined:
            # route each qid into the appropriate store; the other gets NaN
            for s in streams:
                for r in region_names:
                    if sel_fooled:
                    ...
                    if sel_robust:
                        store_V2[s][r].append(pv_cur.get(s, {}).get(r, nan_vec()))
                        store_N2[s][r].append(pn_cur.get(s, {}).get(r, nan_vec()))
                    else:
                        store_V2[s][r].append(nan_vec())
                        store_N2[s][r].append(nan_vec())
        else:
    ...
    # ── output: combined_mg → single 2-row x 1-col subfigure per stream ───────
    if is_combined:
        for stream in streams:
            series_fooled = {rn: stack(store_V[stream],  rn) for rn in region_names}
            series_robust = {rn: stack(store_V2[stream], rn) for rn in region_names}
            plot_combined_mg_subsets(
                x=layers,
                series_fooled=series_fooled,
                series_robust=series_robust,
                out_path=out_root / f"combined_mg_{stream}.pdf",
                ylabel="Avg attn per token (region)\n/ avg attn per token (image)",
                ci=args.ci,
                highlight_spans=[(10, 17)],          # shaded band at layers 10–15
                highlight_color="#AA99CC",            # neutral grey; change as needed
                highlight_alpha=0.25,
                highlight_label="Competition Region",      # set None to omit from legend
            )

    # ── output: standard single-subset plots (unchanged) ─────────────────────
    else:
        for stream in streams:
            ...
            plot_three_lines_with_ci(
                ...
                x=layers,
                series=series,
                out_path=out_root / f"subset_only_{stream}_{args.subset}.pdf",
                ylabel="Avg attn per token (region)\n/ avg attn per token (image)",
                ci=args.ci,
            )
        ...
        for stream in streams:
            ...
            plot_notext_only_with_ci(
                ...
                out_path=out_root / f"subset_notext_only_{stream}_{args.subset}.pdf",
                ylabel="Avg attn per token (region)\n/ avg attn per token (image)",
                ci=args.ci,
            )
        ...
        for stream in streams:
            ...
            plot_three_lines_with_ci(
                ...
                x=layers,
                series=series_delta,
                out_path=out_root / f"subset_delta_vs_notext_{stream}_{args.subset}.pdf",
                ylabel="Δ[avg attn per token (region)\n/ avg attn per token (stream)]",
                ci=args.ci,
            )
    ...
    if is_combined:
        for stream in streams:
            outputs[f"combined_mg_{stream}"] = f"combined_mg_{stream}.pdf"
    else:
        for stream in streams:
            outputs[f"subset_only_{stream}_{args.subset}"] = f"subset_only_{stream}_{args.subset}.pdf"
            outputs[f"subset_notext_only_{stream}_{args.subset}"] = f"subset_notext_only_{stream}_{args.subset}.pdf"
            outputs[f"subset_delta_vs_notext_{stream}_{args.subset}"] = f"subset_delta_vs_notext_{stream}_{args.subset}.pdf"
